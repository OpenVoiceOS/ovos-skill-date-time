...
class TimeSkill(OVOSSkill):
    ...
    def _load_locale_phrase_set(self, name: str):
        ...
        spoke `skill.error` instead of answering. Six of the 22 locales
        ship none of these files, so the empty set is the normal case
        there, not a defect: an empty set means the locale marks no phrase,
        ...
        if cache_key not in self.resources.static:
            self.resources.static[cache_key] = {
                ...
                for phrase in (self.resources.load_list_file(name) or [])
                if phrase.strip()
            }
