...
class TimeSkill(OVOSSkill):
    ...
    def _resolve_month_number(self, month_name: str) -> Optional[int]:
        """Map a spoken month name or abbreviation to its calendar number."""
        return self._MONTH_NUMBERS.get(month_name.strip().lower())

    @intent_handler("days_in_month.intent")
    def handle_days_in_month(self, message):
        """Speak how many days a named month has in the current year."""
        month_number = self._resolve_month_number(message.data.get("month", ""))
        if not month_number:
            self.speak_dialog("extract.date.error")
            return
        now = self.get_datetime()  # session aware
        num_days = calendar.monthrange(now.year, month_number)[1]
        self.speak_dialog("days_in_month", {
            "month": nice_month(now.replace(month=month_number, day=1),
                                lang=self.lang),
            "num_days": num_days})
